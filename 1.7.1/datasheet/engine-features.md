---
type: page
title: Engine Features
listed: false
description: 
index_title: Engine Features
hidden: true
keywords: 
tags: 
---

ads  {% icon classes="fas fa-check-circle" /%} adsfa  {% icon classes="far" /%}

{% table layout="auto" %}
{% row %}
{% cell header=true colwidth=[378] %}
Engine Feature
{% /cell %}
{% cell header=true colwidth=[92] %}
OPSWAT FileScan
{% /cell %}
{% cell header=true colwidth=[92] %}
Cloud Online Analysis Tool "A"
{% /cell %}
{% cell header=true colwidth=[101] %}
Cloud Online Analysis Tool "B"
{% /cell %}
{% cell header=true %}
Cloud Online Analysis Tool "C"
{% /cell %}
{% cell header=true %}
Cloud Online Analysis Tool "D"
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Render URLs and Detect Phishing Sites
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract and Decode Nearly All Malicious VBA Macros
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Analyze VBA Stomped Files Targeted for Any System
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Shellcode Emulation (x86, 32/64)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Export MISP (JSON) and STIX Report Formats
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract and Analyze Embedded PE Files
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Deobfuscate Javascript/VBS
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
Limited
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Deobfuscate Powershell Scripts
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
Limited
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Deobfuscate MSHTA Scripts
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Parse METF Embed Equation Exploit Structure
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Parse Malformed RTF Files
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Parse Office Binary File Formats (BIFF5/BIFF8)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Parse Strict OOXML File Format
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Automatically Decode Embedded Base64 Strings
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract Annotated Disassembly
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Decrypt Password Protected Office Documents
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Decompile Java
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Decompile .NET
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate .NET GUIDs (Module Version/TypeLib Id)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Classify Imported APIs
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
MITRE ATT\&CK Support (In-report and Search)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Render PDF Pages
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract Embedded Files (eg: OLE2 from Word)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Automatically Tag Samples Based on Signatures
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
YARA Support
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Generate Text Metrics (Average Word Size, etc.)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Detect Cryptographic Constants
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Text Analysis (Guessed Language)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Map UUIDs to Known Associated Files / Metadata
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
Limited
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Filter Strings and Detect Interesting Ones
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract and Detect Overlay
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Integrated Allowlist
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Detect Alternative IOCs (Emails, Bitcoin Address, etc.)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate Authentihash
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Verify Authenticode Signatures
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Parse RICH Header
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
Limited
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate Entropy of Resources
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Detect URLs, Domains and IP Addresses
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
Limited
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate Hashes of Resources
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate Imphash
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate SSDEEP
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract PDB Information
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Detect TLS Callbacks
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Resolve Known Import Ordinals to Names
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% p /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Detect Anomalies (eg: Header Checksum Validation)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
Limited
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Query VirusTotal for Reputation Checks
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Detect Packers (PEiD)
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Detect File Types
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate Hashes of Sections
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Calculate Entropy of Sections
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract Strings from Executable
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract/Detect Resources
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% row %}
{% cell %}
Extract/Detect PKCS7 Certificate
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% cell %}
{% icon classes="fas fa-check-circle" /%}
{% /cell %}
{% /row %}
{% /table %}
