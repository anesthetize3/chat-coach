...
Make sure that there is a working internet connection and the OPSWAT Activation Server is reachable ([https://activation.dl.opswat.com](https://activation.dl.opswat.com))
...
Have a look on the `<installdir>/fsTransform/fstransform.log` file for more error details, example:

{% code %}
```bash {% title="License error example log" %}
