...
To achieve the maximum possible throughput on a single server, make sure to deploy Filescan on a powerful instance:
...
# Disable expensive/unpreditable subtasks 
...
Log in to Filescan using an admin user, and open the Admin Panel to change the following settings.
