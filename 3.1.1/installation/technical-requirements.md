...
- **Ubuntu Server 22.04** LTS ("Jammy Jellyfish") or **Ubuntu Server 24.04** LTS ("Noble") or **Red Hat Enterprise Linux 9.x** or **Rocky Linux 9.x**:
  ...
  - Please download the Ubuntu ISO image from: [https://releases.ubuntu.com/noble/](https://releases.ubuntu.com/noble/)

{% callout type="warning" title="Plan for legacy OS support" %}
OPSWAT will discontinue support for the following OS in MetaDefender Aether/Sandbox:

- **Ubuntu 22.04** from **Apr 2027**.
{% /callout %}
...
- 256 GB **Free Disk Space** (SSD)\_: Sandbox requires 32 GB on the first installation (subsequent upgrades will potentially consume more disk space). More information for *\_multi-partition layouts*\*: [Low disk space](../troubleshooting/low-disk-space.md)
  ...
  - \*Disk space recommendation is assuming default retention settings and an average file size is assumed to be in the single digit MB range. If very large files are sent at large quantities, total disc size has to be much larger or retention settings more strict.
- Internet access is required during the installation \& upgrade process, except for [Offline Installation](offline-installation.md).
...
{% /cell %}
{% cell header=true %}
Red Hat 9.5 (or newer)
{% /cell %}
{% cell header=true %}
Rocky Linux 9.5 (or newer)
{% /cell %}
{% /row %}
{% row %}
{% cell %}
...
{% row %}
{% cell %}
Install on CIS level 2 hardened system
{% /cell %}
{% cell %}
**✓**
{% /cell %}
