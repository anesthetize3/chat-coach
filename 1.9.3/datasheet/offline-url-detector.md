...
The offline URL detection ML model enhances security by effectively identifying suspicious URLs. Unlike traditional methods, it utilizes machine learning on a dataset of labelled URLs to accurately detect threats without relying on web rendering or code analysis. Its adaptability to offline environments makes it versatile for various security scenarios, such as preventing malware from exfiltrating data through disguised URLs or thwarting phishing attempts by flagging fraudulent links in emails.

## Data

The Suspicious URL detector ML model was trained on close to 1 million URLs from various sources, including reputation vendors and feeds. The ratio of benign to malicious URLs  in the dataset was approximately 1 to 4.
...
{% callout title="Info" %}
The default setting enables the URL model, which can only provide an assessment of whether a URL is suspicious, without offering a more definitive judgment.
{% /callout %}
